...
def rebalance_loader(dataset, args, model=None, use_true_target=False, device=None, target_dataset=None, save_path = None):
    ...
    if use_true_target:
        ...
        target = infer_predictions(model, dataloader, device, save_path=save_path)
        model.train()
...
def infer_predictions(model, loader, device, epoch=None, save_path="new_our-pseudo-labels-metrics.csv"):
    ...
    with torch.no_grad():
        ...
        print(f"GM: {gm:.4f} | bACC: {bACC:.4f} | Precision: {precision:.4f} | F1: {f1:.4f}")
...
def main(args: argparse.Namespace):
    ...
    args.class_names = [0, 1, 2, 3]
    args.num_classes = 4

    train_source_dataset = datapreprocessing.HAM10000_Dataset(annotations_file='../data/metadata/H_4.csv', img_dir='../data/HAM',
                                                              transform=train_source_transform)
    train_target_dataset = datapreprocessing.PAD_Dataset(annotations_file='../data/metadata/P_4.csv', img_dir='../data/PAD',
                                                         transform=train_target_transform)
    test_dataset = val_dataset = datapreprocessing.PAD_Dataset(annotations_file='../data/metadata/P_4.csv', img_dir='../data/PAD',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--targetimage', type=str, default='../data/HLH',
    ...
    parser.add_argument('--e_cutoff', default=-11.5, type=float, help='INPL')
